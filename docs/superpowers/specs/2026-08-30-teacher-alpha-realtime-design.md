...
### `alpha_records` (알파 수업 기록 · 학생×날짜)
```
id           uuid pk
code         text        -- timetable.code 매칭
name_kor     text
date         date default current_date
program      text default '알파'
...
unique(code, date)
```
- 교재배부는 우선 이 표의 `distributed`에 기록(추후 청구 시스템 연동은 별도).
...
- 명단 각 학생 행: 출결 · 학습교재 · 진도 · 결과 · 특이사항 · 교재배부 입력 → `alpha_records` upsert.
- 강사 일지(비공개, 반 단위) — `class_daily` 재사용.
- 도구 카드: 교재배부(AppSheet 앱 링크) 등.
- 데모 폴백: Supabase 연결 없으면 샘플 명단.

### 2. 원장 시간표 편집기 (`admin.html`에 섹션 추가) — MVP
...
| 단계 | 내용 | 실행 주체 |
|---|---|---|
| 1 | `timetable`·`alpha_records` 생성 + Realtime + RLS SQL | 원장님(Supabase SQL 에디터 1회) |
| 2 | 알파 학생 시드 INSERT (시트에서 추출·거친 이관) | 원장님 1회 |
| 3 | `teacher-alpha.html` 실시간 명단 + 기록 저장 | 배포 |
| 4 | 원장 시간표 편집기 | 배포 |
| 5 | 검증(원장 편집 → 교실 화면 즉시 반영) | — |

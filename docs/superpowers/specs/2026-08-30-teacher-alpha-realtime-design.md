...
- **교재배부 링크**: AppSheet 앱 **배부 입력** 뷰로 deep-link (`#view=배부 입력`). 열자마자 배부 입력 폼.
- **강사 로그인 통합**: 원어민·알파·문법·독해 전 강사가 **같은 아이디/비번**. 홈 → 강사용 → 로그인(1회) → `teacher-hub.html`에서 각자 페이지 선택. Supabase 세션 동일 origin 공유 → 페이지들은 재로그인 없이 진입.
- **원장/관리자 페이지 재구성**(큰 개편, 별도 단계):
  - 메인 = **전체 학생 리스트 한 페이지**.
  - 오른쪽 **사이드바 대시보드**: 학생관리 · 직원관리 · 보강관리 · 교재관리 · 학습관리(월말레포트·영상기록·출결기록).
  - 상단: 상담 일정 · 보강 일정 · 전체 수업.

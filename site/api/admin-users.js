...
// 인증: 호출자의 Supabase 세션 토큰(Authorization 헤더) 검증 → ADMINS 만 통과. (consult.js 와 동일 패턴)
// ponytail: 시드는 offset 25건씩 나눠 처리(서버리스 타임아웃 회피).
...
export default async function handler(req, res){
  ...
  try{
    if(b.action==="list"){
      const rows = (await listAll()).map(x=>({ id:x.id, email:x.email, name:(x.user_metadata&&x.user_metadata.name)||"", subj:(x.user_metadata&&x.user_metadata.subj)||"" }))
